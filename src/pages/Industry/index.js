...
const PageIndustry = () => {
  ...
  const handleGetRows = useCallback(async () => {
    const result = await serviceIndustry.getAllItems();
    if (result) {
  ...
  return (
    <div>
      ...
      <Drawer anchor="right" open={drawerState} onClose={toggleDrawer(false)}>
        ...
        >
          <FormSection
            ...
            handlePost={serviceIndustry.postItem}
            handlePut={serviceIndustry.putItem}
      ...
      <Box sx={{ mt: 5 }}>
        <Typography variant="h6">Todas as Industries</Typography>
        ...
        {activeLoaging ? (
          ...
          <GridListItens
            listRows={rows}
            handleList={handleGetRows}
            handleDelete={serviceIndustry.deleteItem}
            OpenAlertMensage={handleOpenMenssage}
            columns={columns}
